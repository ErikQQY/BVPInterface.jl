...
macro import_bvpsol()
  :(
    using BVPInterface: bvpsol, bvpsol_i32
  )
end
...
macro import_DLbvpsol()
  :(
    using BVPInterface: DL_BVPSOL, DL_BVPSOL_I32
  )
end
...
macro import_bvpsol_help()
  :(
    using BVPInterface: help_bvpsol_compile, help_bvpsol_license
  )
end
...
"""
  ...
  `retcode` can have the following values:
        ...
        -7: wrong EPS (should not happen; checked by BVPInterface module)
...
"""
  ...
  complete BVPInterface packet. Apart from that, you should not
