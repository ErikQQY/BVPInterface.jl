# Dynamic loading of ODE-Solvers for BVPInterface
...
macro import_dynamicload()
  :(
    using BVPInterface: loadODESolvers, unloadODESolvers
  )
end
...
"""
  ...
  solver lib-name in the BVPInterface_jll module.

  The BVPInterface_jll modules must be loaded before.
...
function trytofindjlllib(name::AbstractString)
  ...
  ptr = getproperty(BVPInterface_jll, ptr_name) :: Ptr{Nothing}
  filepath = getproperty(BVPInterface_jll, filepath_name) :: AbstractString
  return (ptr, filepath)
end
...
"""
  ...
  ## case: use BVPInterface_jll
  If the julia version is 1.3 or higher and `ignore_jll` is false (the default)
  then this method only tries to use `BVPInterface_jll`. In this case
  the extrapaths-argument is ignored/useless.

  ## case: don't use `BVPInterface_jll`
  ...
  find the path of the BVPInterface module and (if successfull)
  ...
  form of the result or call `help_solversupport()`.
       ...
       BVPInterface.help_solversupport()
  """
function loadODESolvers(extrapaths::Vector=AbstractString[],
          loadlibnames::Tuple=(); ignore_jll=false )
    ...
    try
      ...
      if load_tried
        throw(SolverODEnotLoaded(string(
          ...
          "Please check BVPInterface.help_solversupport() and ",
          "call loadODESolvers and check also this output. ",
          "For further information see also BVPInterface.help_install.")))
      else
